...
class Pokemon:
    def __init__(self, name, baseHp, lifePerLevel, attack, attackPerLevel, baseDef, defencePerLevel, spells, element):
        self.level = 1
        self.exp = 0
        self.name = name
        self.baseHp = baseHp
...
#        return 1.2 * (self.level**3) - 15 * (self.level**2) + 100 * self.level - 140
    ...
    def str(self):
        ...
        return "(" + str(self.pokeid) + ", " + self.name + ", Niveau(" + str(self.level) + "), Attaque(" + str(self.getAttack()) + "), VieMax(" + str(self.getMaxLife()) + "), Defense(" + str(self.getDefence()) + "), Sorts: (" + spells + "), Exp: (" + str(self.exp) + " / " + str(self.expForNextLevel()) + "))"
    ...
    def __init__(self):
        self.pokemons = [
            Pokemon("ZeratoR", 140, 6, 10, 1, 5, 0.1, [
                    Spell("Son_Pere", 10, 90, elementsManager.get("Feux")),
                    Spell("Mute", 15, 50, elementsManager.get("Feux")),
                    Spell("Rend_l'argent", 50, 100, elementsManager.get("Feux")),
                    Spell("Dailymotion_drop", 100, 100, elementsManager.get("Feux"))
                    ], elementsManager.get("Feux")),
            ...
            Pokemon("MisterMV", 140, 6, 9, 1, 0.1, 0.1, [
                    ...
                    Spell("Speedrun", 20, 80, elementsManager.get("Feux")),
                    Spell("Jeu_a_la_pisse", 100, 30, elementsManager.get("Terre"))
                    ], elementsManager.get("Terre")),
            Pokemon("Leeroy Jenkins", 100, 5, 20, 1, 0.1, 0.1, [
                    Spell("LEEEEEROY_JENKINS", 5000, 10, elementsManager.get("Feux"))
                    ], elementsManager.get("Feux")),
            ...
            Pokemon("Demoneth", 160, 5, 10, 1, 4, 0.2, [
                    Spell("Molotov_sur_orange", 20, 50, elementsManager.get("Feux")),
                    Spell("Live_o_maniaque", 15, 100, elementsManager.get("Feux")),
                    Spell("La_co_marche", 100, 10, elementsManager.get("Feux"))
                    ], elementsManager.get("Feux"))
            ]
